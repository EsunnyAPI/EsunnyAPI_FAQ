...
普通的期货、期权合约，订阅时只需要1个合约。此时只需要填写：合约代码1、执行价1、看涨看跌标示1。

对于跨期套利、跨品种套利类型的合约，需要有两个合约。测试分别填写：合约代码1、执行价1、看涨看跌标示1，合约代码2、执行价2、看涨看跌标示2。

注意：档只有1个合约时，看涨看跌标示2要设置为 TAPI_CALLPUT_FLAG_NONE 。
